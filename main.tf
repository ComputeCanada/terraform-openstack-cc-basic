...
# Load Balancer/Router node: persistent with root disk and floating IP
...
resource "openstack_networking_floatingip_v2" "fip" {
  ...
  pool = "${var.pp_spec["floating_ip_pool"]}"
}
...
resource "openstack_compute_instance_v2" "pp_node" {
  ...
  provision "local-exec" {
    # TODO: use variable for config file
    command = "echo 'Host ${element(openstack_compute_instance_v2.pp_node.*.name, count.index)}\n\tHostname ${element(openstack_compute_instance_v2.pp_node.*.network.1.fixed_ip_v4, count.index)}\n\tProxyCommand ssh -q -W %h ${var.pp_spec["jumphost"]}\n' >> ssh_config"
  }
}
